...
// Modul leaderboard.c
// Berfungsi menyimpan skor, membaca skor, menampilkan leaderboard,
// dan fitur pencarian nama leaderboard.

// Mengamankan nama agar tidak mengandung karakter yang mengganggu penyimpanan (misalnya koma)
void safe_name(char *s) {
    for (int i=0;s[i];i++) if (s[i]==',') s[i]='_';
}

// Menyimpan skor ke dalam file CSV
...
// Fungsi pembanding untuk sorting leaderboard
...
void show_leaderboard(void) {
    ...
    // Membaca semua baris pada file CSV
