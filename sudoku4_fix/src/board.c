...
// Modul board.c
// Berisi fungsi-fungsi untuk mengelola papan Sudoku 4x4:
// - representasi solusi dasar,
// - cetak papan,
// - cek validasi angka,
...
// Solusi dasar Sudoku 4x4 (papan sudah terisi lengkap dan valid)
...
// Menyalin isi papan dari src ke dst
...
// Mencetak satu baris papan (versi rekursif)
void print_row_rec(int a[N][N], int row, int col) {
    if (col == N) return; 

    if (a[row][col] == 0) printf(" .");
    ...
    print_row_rec(a, row, col + 1);
}

// Menampilkan papan Sudoku lengkap beserta waktu dan skor
...
// Mengecek apakah nilai v valid jika diletakkan di sel (r, c)
...
// Rekursif: menghitung jumlah solusi yang mungkin (backtracking)
...
// Wrapper untuk menghitung jumlah solusi dari suatu papan
...
// Membuat puzzle dari papan solusi lengkap dengan mengosongkan beberapa sel
void generate_puzzle(int a[N][N], int blanks) {
    ...
    for (int k=0;k<N*N && removed<blanks;k++) {
        ...
        int saved=a[r][c];
        a[r][c]=0;
        // Pastikan puzzle tetap memiliki tepat satu solusi
